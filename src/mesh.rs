...
impl Wireframe {
    ...
    pub fn edges(&self) -> Vec<LineSegment3> {
        ...
        for (&from_index, &to_index) in self.indices.iter().tuples() {
            let from = Point3::new(
                self.vertices[from_index * 3],
                self.vertices[from_index * 3 + 1],
                self.vertices[from_index * 3 + 2],
            );

            let to = Point3::new(
                self.vertices[to_index * 3],
                self.vertices[to_index * 3 + 1],
                self.vertices[to_index * 3 + 2],
            );
